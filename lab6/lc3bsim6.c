...
//Signal generated by MEM Stage
uint8_t mem_drid;
uint8_t mem_pcmux;
uint16_t target_pc;
uint16_t trap_pc;
uint8_t v_mem_ld_cc;
uint8_t v_mem_ld_reg;

//Signals generated by AGX stage
uint8_t agex_drid;
uint8_t v_agex_ld_cc;
uint8_t v_agex_ld_reg;
...
void MEM_stage() {
  ...
  if (PS.MEM_V == 1) {
    if (Get_TRAP_OP(PS.MEM_CS) == 1) {
        mem_pcmux = 2;
    } else if(Get_UNCOND_OP(PS.MEM_CS) | (Get_BR_OP(PS.MEM_CS) && ((((PS.MEM_IR & 0x0E00) >> 9) & PS.MEM_CC) != 0))) {
        mem_pcmux = 1;
    } else {
        mem_pcmux = 0;
    }
  ...
  int read_word;
  uint8_t mem_address_0 = PS.MEM_ADDRESS & 0b1;
  int write_word=0;
  int mem_w0=0, mem_w1=0;
  ...
  if (Get_DATA_SIZE(PS.MEM_CS)) {
    ...
    if (mem_address_0 == 0) {
        trap_pc = signExtendTo16((read_word & 0x00FF), 8);   
    } else {
        trap_pc = signExtendTo16((read_word & 0xFF00) >> 8, 8);
    }
  }
  ...
  if (1) {
    ...
    NEW_PS.SR_V = PS.MEM_V && (mem_stall==0);
  }
...
void AGEX_stage() {
  ...
  uint16_t addr2mux_out;

  if (Get_ADDR2MUX(PS.AGEX_CS) == 0) {
    addr2mux_out = 0;
  } else if (Get_ADDR2MUX(PS.AGEX_CS) == 1) {
    addr2mux_out = signExtendTo16(PS.AGEX_IR & 0x003F, 6);
  } else if (Get_ADDR2MUX(PS.AGEX_CS) == 2) {
    addr2mux_out = signExtendTo16(PS.AGEX_IR & 0x01FF, 9);
  } else {
    addr2mux_out = signExtendTo16(PS.AGEX_IR & 0x07FF, 11);
  } 

  uint16_t alu_out;

  if (Get_ALUK(PS.AGEX_CS) == 0) {
    alu_out = PS.AGEX_SR1 + (Get_SR2MUX(PS.AGEX_CS)?signExtendTo16(PS.AGEX_IR & 0x001F,5): PS.AGEX_SR2);
  } else if (Get_ALUK(PS.AGEX_CS) == 1) {
    alu_out = PS.AGEX_SR1 & (Get_SR2MUX(PS.AGEX_CS)?signExtendTo16(PS.AGEX_IR & 0x001F,5): PS.AGEX_SR2);
  } else if (Get_ALUK(PS.AGEX_CS) == 2) {
    alu_out = PS.AGEX_SR1 ^ (Get_SR2MUX(PS.AGEX_CS)?signExtendTo16(PS.AGEX_IR & 0x001F,5): PS.AGEX_SR2);
  } else {
    alu_out = (Get_SR2MUX(PS.AGEX_CS)?signExtendTo16(PS.AGEX_IR & 0x001F,5): PS.AGEX_SR2);
  }
  ...
  uint8_t lshf1 = Get_LSHF1(PS.AGEX_CS);

  int shift_dir, shift_am;
  int shift;
  shift_dir = (PS.AGEX_IR & 0x0030) >> 4;
  shift_am = PS.AGEX_IR & 0x000F;

  if (shift_dir == 1)
    shift = Low16bits(((uint16_t)PS.AGEX_SR1) >> shift_am);
  else if (shift_dir == 3)
    shift = Low16bits(((int16_t)PS.AGEX_SR1) >> shift_am);
  else 
    shift = Low16bits(PS.AGEX_SR1 << shift_am);

  LD_MEM = (!mem_stall); 
 
  if (LD_MEM) {
    /* Your code for latching into MEM latches goes here */
    NEW_PS.MEM_ADDRESS =  Low16bits(Get_ADDRESSMUX(PS.AGEX_CS)? \
                            (Get_ADDR1MUX(PS.AGEX_CS)? PS.AGEX_SR1: PS.AGEX_NPC) + (addr2mux_out << lshf1): \
                            (PS.AGEX_IR & 0x00FF) << 1);

    NEW_PS.MEM_NPC = PS.AGEX_NPC;
    NEW_PS.MEM_CC = PS.AGEX_CC;
    NEW_PS.MEM_ALU_RESULT = Get_ALU_RESULTMUX(PS.AGEX_CS)? Low16bits(alu_out): shift;
...
void DE_stage() {
  ...
  v_de_br_stall = PS.DE_V & CONTROL_STORE[CONTROL_STORE_ADDRESS][15];

  uint8_t sr1_id, sr2_id;
  sr1_id = (PS.DE_IR & 0x01C0) >> 6;
  sr2_id = ((PS.DE_IR >> 13) & 0b1)? ((PS.DE_IR & 0x0E00) >> 9): (PS.DE_IR & 0x0007);

  uint16_t agex_sr1_id = REGS[sr1_id];
  uint16_t agex_sr2_id = REGS[sr2_id];

  if (v_sr_ld_reg == 1) {
    REGS[sr_reg_id] = Low16bits(sr_reg_data);
  }

  uint8_t sr1_needed = CONTROL_STORE[CONTROL_STORE_ADDRESS][SR1_NEEDED];
  uint8_t sr2_needed = CONTROL_STORE[CONTROL_STORE_ADDRESS][SR2_NEEDED];
  uint8_t de_br_op = CONTROL_STORE[CONTROL_STORE_ADDRESS][BR_OP];

  if ( sr1_needed && v_agex_ld_reg && (sr1_id == agex_drid) )
    dep_stall = PS.DE_V;
  else if ( sr1_needed && v_mem_ld_reg && (sr1_id == mem_drid) )
    dep_stall = PS.DE_V;
  else if ( sr1_needed && v_sr_ld_reg && (sr1_id == sr_reg_id) )
    dep_stall = PS.DE_V;
  else if ( sr2_needed && v_agex_ld_reg && (sr2_id == agex_drid) )
    dep_stall = PS.DE_V;
  else if ( sr2_needed && v_mem_ld_reg && (sr2_id == mem_drid) )
    dep_stall = PS.DE_V;
  else if ( sr2_needed && v_sr_ld_reg && (sr2_id == sr_reg_id) )
    dep_stall = PS.DE_V;
  else if ( de_br_op && ( v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc ) )
    dep_stall = PS.DE_V;
  else
    dep_stall = 0;
  ...
  if (LD_AGEX) {
    ...
    NEW_PS.AGEX_SR1 = agex_sr1_id;
    NEW_PS.AGEX_SR2 = agex_sr2_id;

    NEW_PS.AGEX_CC = (N_pre << 2) + (Z_pre << 1) + P_pre;
    NEW_PS.AGEX_DRID = (CONTROL_STORE[CONTROL_STORE_ADDRESS][2])?7:((PS.DE_IR & 0x0E00)>>9);
