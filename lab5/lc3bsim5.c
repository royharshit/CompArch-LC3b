...
enum CS_BITS {                                                  
    ...
    ALUBMUX1, ALUBMUX0,
    MARMUX21, MARMUX20,
    ISR_ADDRMUX2, ISR_ADDRMUX1, ISR_ADDRMUX0,
    SR1MUX1,
    DRMUX1,
    PSR15MUX,
    CLR_INT,
...
int GetALUBMUX(int *x)       { return((x[ALUBMUX1] << 1) + x[ALUBMUX0]); }
int GetMARMUX2(int *x)       { return((x[MARMUX21] << 1) + (x[MARMUX20])); }
int GetISR_ADDRMUX(int *x)   { return((x[ISR_ADDRMUX2] << 2) + (x[ISR_ADDRMUX1] << 1) + x[ISR_ADDRMUX0]); }
...
void cycle() {                                                
  ...
  if ((CYCLE_COUNT == 300) && ((CURRENT_LATCHES.PSR & 0x8000) >> 15)) {
...
void eval_micro_sequencer() {
   ...
   */
    ...
    int ird1 = CURRENT_LATCHES.MICROINSTRUCTION[IRD1]; 
    int ird2 = CURRENT_LATCHES.MICROINSTRUCTION[IRD2]; 
    int cond2 = CURRENT_LATCHES.MICROINSTRUCTION[COND2]; 
    ...
    int ir_14 = (CURRENT_LATCHES.IR >> 14) & 0b1; 
    int opcode = (CURRENT_LATCHES.IR >> 12) & 0xF; 
    int opcode1 = (CURRENT_LATCHES.IR >> 12) & 0xD; 

    uint16_t psr15 = ((CURRENT_LATCHES.PSR & 0x8000) >> 15);
    uint8_t interupt = CURRENT_LATCHES.INTV & 0x0001;
    uint8_t p = ((CURRENT_LATCHES.MDR & 0x0008) >> 3);
    uint8_t v = ((CURRENT_LATCHES.MDR & 0x0004) >> 2);
    uint8_t unaligned = CURRENT_LATCHES.VA & 0x0001;
    uint8_t fetch = CURRENT_LATCHES.FETCH; 

    NEXT_LATCHES.STATE_NUMBER = ird ? fetch? 0x28 : ird1 ? (48+opcode1): ird2? 0x14 + ir_11: opcode: \
        (((CURRENT_LATCHES.MICROINSTRUCTION[J5] & 0b1) | ((p == 0) & (psr15 == 1) & (cond2 == 1) & (cond1 == 1) & (cond0 == 0))) << 5) + \
        (((CURRENT_LATCHES.MICROINSTRUCTION[J4] & 0b1) | ((unaligned == 1) & (ir_14 == 1) & (cond2 == 1) & (cond1 == 0) & (cond0 == 1))) << 4) + \
        (((CURRENT_LATCHES.MICROINSTRUCTION[J3] & 0b1) | ((interupt == 1) & (psr15 == 1) & (cond2 == 1) & (cond1 == 1) & (cond0 == 1))) << 3) + \
        (((CURRENT_LATCHES.MICROINSTRUCTION[J2] & 0b1) | ((ben == 1) & (cond2 == 0) & (cond1 == 1) & (cond0 == 0))) << 2) + \
        (((CURRENT_LATCHES.MICROINSTRUCTION[J1] & 0b1) | ((r == 1) & (cond2 == 0) & (cond1 == 0) & (cond0 == 1))) << 1) + \
        (((CURRENT_LATCHES.MICROINSTRUCTION[J0] & 0b1) | ((v == 0) & (cond2 == 0) & (cond1 == 1) & (cond0 == 1))));
...
int16_t gate_marmux_input, gate_pc_input, gate_alu_input, gate_shf_input, gate_mdr_input, gate_pc1_input, gate_psr_input;
int16_t adder;
...
void eval_bus_drivers() {
  ...
  // Gate MARMUX Input
  int16_t sr1 = (CURRENT_LATCHES.MICROINSTRUCTION[SR1MUX1]==0)? \
                (CURRENT_LATCHES.MICROINSTRUCTION[SR1MUX]==1)? CURRENT_LATCHES.REGS[(CURRENT_LATCHES.IR>>6)&0x7]: CURRENT_LATCHES.REGS[(CURRENT_LATCHES.IR>>9)&0x7]: \
  ...
  //Gate PC1 Input
  gate_pc1_input = CURRENT_LATCHES.PC -2;

  //Gate PSR Input
  gate_psr_input = CURRENT_LATCHES.PSR;
  ...
  if (GetALUBMUX(CURRENT_LATCHES.MICROINSTRUCTION)==0) {
    sr2 = ((CURRENT_LATCHES.IR >> 5)&0b1)? signExtendTo16((CURRENT_LATCHES.IR&0x1F), 5): CURRENT_LATCHES.REGS[(CURRENT_LATCHES.IR)&0x7]; 
  } else if (GetALUBMUX(CURRENT_LATCHES.MICROINSTRUCTION)==1) {
    sr2 = 2;
  } else if (GetALUBMUX(CURRENT_LATCHES.MICROINSTRUCTION)==2) {
    sr2 = -2;
  }
...
void drive_bus() {
  ...
  } else if (CURRENT_LATCHES.MICROINSTRUCTION[GATE_PC1] == 1) {
        BUS = Low16bits(gate_pc1_input);
  } else if (CURRENT_LATCHES.MICROINSTRUCTION[GATE_PSR] == 1) {
...
void latch_datapath_values() {
  ...
  if (CURRENT_LATCHES.MICROINSTRUCTION[CLR_INT] == 1) {
    NEXT_LATCHES.INTV = 0;
  }
  ...
  uint8_t ld_ssp = CURRENT_LATCHES.MICROINSTRUCTION[LD_SSP];
  uint8_t ld_isr = CURRENT_LATCHES.MICROINSTRUCTION[LD_ISR_ADDR];
  uint8_t ld_psr = CURRENT_LATCHES.MICROINSTRUCTION[LD_PSR];
  uint8_t ld_psr15 = CURRENT_LATCHES.MICROINSTRUCTION[LD_PSR15];
  uint8_t ld_va = CURRENT_LATCHES.MICROINSTRUCTION[LD_VA];
  uint8_t ld_mr = CURRENT_LATCHES.MICROINSTRUCTION[LD_MR];
  uint8_t fetch_set = CURRENT_LATCHES.MICROINSTRUCTION[FETCH_SET];
  uint8_t fetch_reset = CURRENT_LATCHES.MICROINSTRUCTION[FETCH_RESET];

  if (fetch_set == 1) {
    NEXT_LATCHES.FETCH = 1;
  }

  if (fetch_reset == 1) {
    NEXT_LATCHES.FETCH = 0;
  }
  ...
  if (ld_mr == 1) {
    NEXT_LATCHES.MR = (CURRENT_LATCHES.MICROINSTRUCTION[MRMUX] == 1)? 0x0003: 0x0001;
  }

  if (ld_psr == 1) {
    NEXT_LATCHES.PSR = Low16bits(BUS);
    NEXT_LATCHES.N = (NEXT_LATCHES.PSR >> 2) & 0b1;
    NEXT_LATCHES.Z = (NEXT_LATCHES.PSR >> 1) & 0b1;
    NEXT_LATCHES.P = NEXT_LATCHES.PSR & 0b1;
  }

  if (ld_psr15 == 1) {
    NEXT_LATCHES.PSR = Low16bits((CURRENT_LATCHES.MICROINSTRUCTION[PSR15MUX]==0)? (CURRENT_LATCHES.PSR & 0x07FFF) | 0x8000: (CURRENT_LATCHES.PSR & 0x07FFF));
  }

  if (ld_isr == 1) {
    if (GetISR_ADDRMUX(CURRENT_LATCHES.MICROINSTRUCTION)==0) {
        NEXT_LATCHES.ISR_ADDR = 0x0202;
    } else if (GetISR_ADDRMUX(CURRENT_LATCHES.MICROINSTRUCTION)==1) {
        NEXT_LATCHES.ISR_ADDR = 0x0204;
    } else if (GetISR_ADDRMUX(CURRENT_LATCHES.MICROINSTRUCTION)==2) {
        NEXT_LATCHES.ISR_ADDR = 0x0206;
    } else if (GetISR_ADDRMUX(CURRENT_LATCHES.MICROINSTRUCTION)==3) {
        NEXT_LATCHES.ISR_ADDR = 0x0208;
    } else {
        NEXT_LATCHES.ISR_ADDR = 0x020A;
    }
  }
  ...
  if (ld_mar == 1) {
    ...
    } else if (GetMARMUX2(CURRENT_LATCHES.MICROINSTRUCTION) == 1) {
        NEXT_LATCHES.MAR = Low16bits(CURRENT_LATCHES.ISR_ADDR);
    } else if (GetMARMUX2(CURRENT_LATCHES.MICROINSTRUCTION) == 2) {
        NEXT_LATCHES.MAR = Low16bits((CURRENT_LATCHES.MDR & 0x3E00) + (CURRENT_LATCHES.VA & 0x01FF));
    } else if (GetMARMUX2(CURRENT_LATCHES.MICROINSTRUCTION) == 3) {
        NEXT_LATCHES.MAR = Low16bits(((CURRENT_LATCHES.MAR >> 9) << 1) + CURRENT_LATCHES.PTBR);
    }
  }
  ...
  if (ld_mdr == 1) {
    ...
    uint8_t mdrmux = CURRENT_LATCHES.MICROINSTRUCTION[MDRMUX]; 
    NEXT_LATCHES.MDR = mdrmux? CURRENT_LATCHES.MDR | CURRENT_LATCHES.MR : \
                        (CURRENT_LATCHES.MICROINSTRUCTION[MIO_EN] == 1)? Low16bits(mem_out): Low16bits(logic_out);     
  }
  ...
  if (ld_reg == 1) {
    uint16_t dr =  (CURRENT_LATCHES.MICROINSTRUCTION[DRMUX1] == 0)? \
                    (CURRENT_LATCHES.MICROINSTRUCTION[DRMUX] == 1)? 7: (CURRENT_LATCHES.IR >> 9) & 0x7 : 6;
    NEXT_LATCHES.REGS[dr] = (CURRENT_LATCHES.MICROINSTRUCTION[RFMUX] == 0)? BUS: CURRENT_LATCHES.SSP;       
  }
  ...
  if (ld_cc == 1) {
    ...
    NEXT_LATCHES.PSR = (NEXT_LATCHES.PSR & 0xFFF8) | ((NEXT_LATCHES.N & 0b1) << 2) | ((NEXT_LATCHES.Z & 0b1) << 1) | (NEXT_LATCHES.P & 0b1);
  }
